...
def create_cost_csv(io, sheetname, csvname):
    cnum = "0" + sheetname[0]
    df = pd.read_excel(
        io, sheet_name=sheetname, skiprows=23, header=None, usecols=range(1, 19)
    )
    columns = [
        "code post",
        "omschrijving post",
...
def costs_vfilt(
    ...
    # factoren invullen
    df.loc["200110", "factor"] = 1.0 / y_hoh_vp
    df.loc["300120", "factor"] = 1.0 / nvp_per_inspectieput
    ...
    totlen = (
        length
        + df.loc["400110", "factor":"opp"].product(axis=0, skipna=True)
        + df.loc["200110", "factor":"opp"].product(axis=0, skipna=True)
    )
    ...
    # Update hoeveelheden
    df.loc[:"80", "hoeveelheid"] = df.loc[:"80", "factor":"opp"].product(
        axis=1, skipna=True
    )
...
def costs_boring(
    length, factor_inspectionwell, nbor, picklename=None, difficulty="normaal"
):
    """
    :param io: path to excelfile
    :param y_hoh_vp: distance between vertical wells
...
def costs_dig(
    length, factor_inspectionwell, nsl, picklename=None, difficulty="normaal"
):
    """
...
def costs_grindkoffer(
    ...
    df.loc["200470", "factor"] = 1.0 / talud_gk
...
def extra_other_costs(df, cnum, difficulty=None):
    ...
    df.loc["BKbdk", "totaal"] = (
        df.iloc[: df.index.get_loc("BKbdk") - 1, -1].dropna().sum()
    )
    df.loc["NTD" + m + "1", "totaal"] = (
        df.loc["NTD" + m + "1", "hoeveelheid"] * df.loc["BKbdk", "totaal"]
    )
    df.loc["BKdk", "totaal"] = (
        df.loc["BKbdk", "totaal"] + df.loc["NTD" + m + "1", "totaal"]
    )
    df.loc["IK" + m + "6", "totaal"] = (
        df.loc["IK" + m + "6", "hoeveelheid"] * df.loc["BKdk", "totaal"]
    )
    df.loc["IK" + m + "8", "totaal"] = (
        df.loc["IK" + m + "8", "hoeveelheid"] * df.loc["BKdk", "totaal"]
    )
    df.loc["IK" + m + "9", "totaal"] = (
        df.loc["IK" + m + "9", "hoeveelheid"] * df.loc["BKdk", "totaal"]
    )
    df.loc["IK" + m + "10", "totaal"] = (
        df.loc["IK" + m + "6" : "IK" + m + "9", "totaal"].dropna().sum()
        + df.loc["BKdk", "totaal"]
    ) * df.loc["IK" + m + "10", "hoeveelheid"]
    df.loc["IK" + m + "11", "totaal"] = (
        df.loc["IK" + m + "6" : "IK" + m + "9", "totaal"].dropna().sum()
        + df.loc["BKdk", "totaal"]
        + df.loc["IK" + m + "10", "totaal"]
    ) * df.loc["IK" + m + "11", "hoeveelheid"]
    df.loc["IK" + m + "12", "totaal"] = (
        df.loc["IK" + m + "6" : "IK" + m + "9", "totaal"].dropna().sum()
        + df.loc["BKdk", "totaal"]
        + df.loc["IK" + m + "10", "totaal"]
    ) * df.loc["IK" + m + "12", "hoeveelheid"]
    df.loc["BKik", "totaal"] = (
        df.loc["IK" + m + "6" : "IK" + m + "12", "totaal"].dropna().sum()
    )
    df.loc["VZBK", "totaal"] = df.loc["BKdk", "totaal"] + df.loc["BKik", "totaal"]
    df.loc["RBK" + m + "3", "totaal"] = (
        df.loc["RBK" + m + "3", "hoeveelheid"] * df.loc["VZBK", "totaal"]
    )
    df.loc["BK" + m, "totaal"] = (
        df.loc["VZBK", "totaal"] + df.loc["RBK" + m + "3", "totaal"]
    )
    df.loc["EK" + m + "2", "totaal"] = (
        df.loc["EK" + m + "2", "hoeveelheid"] * df.loc["VZBK", "totaal"]
    )
    df.loc["EK" + m + "3", "totaal"] = (
        df.loc["EK" + m + "3", "hoeveelheid"] * df.loc["VZBK", "totaal"]
    )
    ...
    df.loc["OK" + m + "1", "totaal"] = (
        df.loc["OK" + m + "1", "hoeveelheid"] * df.loc["VZBK", "totaal"]
    )
    df.loc["OK" + m + "37", "totaal"] = (
        df.loc["OK" + m + "37", "hoeveelheid"] * df.loc["OK" + m + "1", "totaal"]
    )
    df.loc["OBK" + m, "totaal"] = (
        df.loc["OK" + m + "1", "totaal"] + df.loc["OK" + m + "37", "totaal"]
    )
    df.loc["INV" + m, "totaal"] = (
        df.loc["BK" + m, "totaal"]
        + df.loc["EK" + m, "totaal"]
...
def set_difficulty(difficulty, df, cnum, picklename=None):
    ...
    assert (
        difficulty in factors.columns
    ), "difficulty must be one of: ({0}, {1}, {2})".format(*factors.columns[1:])
    df.loc[factors.index, "hoeveelheid"] = factors.loc[:, difficulty]

    return df


def costcalculations(
    logger,
    xsm,
    L_totaal,
    hdrnparams,
    ...
    measures_names = {
        1: "Verticale bronnen",
        2: "Horizontale drain (geboord)",
        3: "Horizontale drain (gegraven)",
        4: "Grindkoffer",
    }
    index_names = [
        1,
        2,
        4,
    ]  # edit to [2, 4] when vertical wells not included in calculations.

    costs_vp = costs_vfilt(
        y_hoh_vp,
        y_length_tot,
        nwells,
        nvp_per_inspectieput,
    ...
    if (
        xsm.z_deklaag_boven - xsm.z_deklaag_onder + 1.0
    ) <= 8.0:  # if drn less than 8 m below surface
        costs_hds = costs_dig(
            y_length_tot,
            1 / 500.0,
            n_hdrns,
    ...
    for j in index_names:
        ...
        if j == index_names[0]:
            finalcosts = idf.loc[
                selected_rows_costs, ("omschrijving post", "totaal")
            ].copy()
            finalcosts.rename(columns={"totaal": measures_names[j]}, inplace=True)
        else:
            finalcosts[measures_names[j]] = idf.loc[
                selected_rows_costs, "totaal"
            ].values
            finalcosts.rename(columns={"totaal": measures_names[j]}, inplace=True)

    finalcosts = finalcosts.iloc[:-1]
    finalcosts["omschrijving post"] = finalcosts["omschrijving post"].apply(
        lambda s: s.replace("Systeem met vertikale filters", "")
    )
    ...
    finalcosts.loc["OHK", "Verticale bronnen"] = (
        0.75 * finalcosts.loc["BK", "Verticale bronnen"]
    )  # 75% van BK, zie rapp.
    finalcosts.loc["OHK", "Horizontale drain (geboord)"] = (
        0.15 * finalcosts.loc["BK", "Horizontale drain (geboord)"]
    )  # 15% van BK, zie rapp.
    finalcosts.loc["OHK", "Grindkoffer"] = (
        0.20 * finalcosts.loc["BK", "Grindkoffer"]
    )  # 20% van BK, zie rapp.
    ...
    finalcosts.iloc[-1, 1:] = 0.0

    # Kosten Kuubs
    kosten_per_kuub_eur = 0.5 / 100.0  # 0,5 eurocent per kuub
    finalcosts.loc[
        "QK", "omschrijving post"
    ] = "Kosten waterbezwaar (€/dag/{0:.0f}m)".format(y_length_tot)
    ...
    if finalcosts.shape[1] == 5:  # includes gegraven drains
        finalcosts = finalcosts.loc[
            :,
            (
                "omschrijving post",
                "Grindkoffer",
                "Horizontale drain (geboord)",
                "Horizontale drain (gegraven)",
                "Verticale bronnen",
            ),
        ]
