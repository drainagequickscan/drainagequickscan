...
from timml.intlinesink import IntFluxDiffLineSink, IntHeadDiffLineSink
...
class HorizontalDrainXsec:
    def __init__(self, xsm, x_drn=0.0, drawdown=1.0, max_head=None):
        self.xsm = xsm
        ...
        h0 = self.xsm.model.head(self.x_drn, 0.0, layers=[0])
        if max_head is not None:
            self.logger.info("Using given maximum head to determine drainage level.")
            self.hls = max_head
            if max_head >= h0:
                raise NoDrainageNeededError(
                    "Vereiste stijghoogte (NAP{0:+.2f} m) ligt hoger dan berekende stijghoogte ".format(
                        max_head
                    )
                    + "in huidige situatie (NAP{0:+.2f} m) bij locatie drainage!".format(
                        h0[0]
                    )
            ...
            self.logger.info(
                "Drainage level is calculated using given drawdown relative "
    ...
    def add_horizontal_drn(self, x_drn, hdrn):
        self.ls = timml.HeadLineSink1D(
            self.xsm.model, xls=x_drn, hls=hdrn, label="horizontale drain"
        )

    def plot_hdrn(self, ax):
        (h,) = ax.plot(
            [self.x_drn, self.x_drn],
            [self.xsm.z_deklaag_onder - 1, self.xsm.z_deklaag_onder],
            c="k",
            lw=4,
            label="horizontale drain",
            ls="solid",
        )
        return h

    def seepage(self):
        Q = np.sum(self.model.disvec(self.xsm.xR_voorland, 0.0))
        return Q
    ...
    def velocity_at_drn(self, porosity=None, diameter=None):
        ...
        return self.discharge() / (2 * np.pi * diameter / 2.0 * 1.0 * porosity)
    ...
    def check_velocity(self, diameter=None, porosity=None):
        ...
        msg = "Velocities: v, vmax, ratio: {0:.2f}, {1:.2f}, {2:.2f}".format(
            v, vmax, ratio
        )
        if ratio > 1.0:
            prefix = "Warning! "
    ...
    def check_pump_needed(self):
        ...
        if b:
            ...
            msg = (
                "Vrije afwatering NIET mogelijk! H_drainage {0:.2f} < H_polder {1:.2f}"
            )
...
class GrindkofferXsec:
    def __init__(
        self, xsm, x_gk=0.0, breedte_gk=1.0, drawdown=1.0, max_head=None, c_gk=1e-3
    ):
        self.xsm = xsm
        self.logger = self.xsm.logger
        self.model = self.xsm.model
        ...
        # determine drain elevation
        h0 = self.xsm.model.head(self.x_gk, 0.0, layers=[0])
        if max_head is not None:
            self.logger.info("Using given maximum head to determine drainage level.")
            self.hls = max_head
            if max_head >= h0:
    ...
    def add_grindkoffer(self, x_gk, breedte_gk, hls):
        ...
        z_gk[0] = z_gk[1]
        inhom_gk = timml.StripInhomMaq(
            self.model,
            x1=x_gk,
            x2=x_gk + breedte_gk,
            kaq=self.xsm.kh,
            z=z_gk,
            c=np.r_[np.atleast_1d(self.c_gk), self.xsm.caq],
            npor=0.3,
            topboundary="semi",
            hstar=hls,
        )
        inh_polder2 = timml.StripInhomMaq(
            self.model,
            x1=x_gk + breedte_gk,
            x2=np.inf,
            kaq=self.xsm.kh,
    ...
    def plot_gk(self, ax, slope_gk=0.33):
        ...
        xgk = (
            np.array(
                [
                    0.0,
                    ddeklaag / slope_gk,
                    ddeklaag / slope_gk + self.breedte_gk,
                    ddeklaag / slope_gk + self.breedte_gk + ddeklaag / slope_gk,
                ]
            )
            - ddeklaag / slope_gk
            + self.x_gk
        ...
        ygk = [
            self.xsm.z_deklaag_boven,
            self.xsm.z_deklaag_onder,
            self.xsm.z_deklaag_onder,
            self.xsm.z_deklaag_boven,
        ]
    ...
    def discharge(self):
        fqgk = (
            lambda x: (self.model.headalongline(x, 0.0, layers=0) - self.hls)
            / self.c_gk
        )
    ...
    def velocity_at_gk(self, porosity=None):
        ...
        return self.discharge() / (self.breedte_gk * 1.0 * porosity)
...
class VerticalWellsXsec:
    def __init__(
        ...
        self.rw = 0.25
        self.yw = np.linspace(
            -self.L_total / 2.0 + self.welldist / 2.0,
            self.L_total / 2.0 - self.welldist / 2.0,
            self.nwells,
        )
        ...
        if self.nwells % 2 == 0:
            ...
            self.ycp = np.mean(
                self.yw[
                    int(np.floor(self.nwells / 2.0)) : int(np.ceil(self.nwells / 2.0))
                    + 1
                ]
            )
        self.wlayers = np.where(
            self.xsm.model.aq.zaqbot >= self.xsm.z_deklaag_onder - self.L_filter
        )[0]
        ...
        # determine drain elevation
        h0 = self.xsm.model.head(0.0, 0.0, layers=[0])
        if max_head is not None:
            self.logger.info("Using given maximum head to determine drainage level.")
            self.hls = max_head
            if max_head >= h0:
        ...
        self.linesink = timml.HeadLineSink1D(
            self.xsm.model,
            xls=xw,
            hls=self.hls,
            label="horizontale drain",
    ...
    def solve(self, tol=0.05, silent=False):
        ...
        while self.hmax - self.hls > tol:
            ...
            self.hmax = self.model2.head(
                self.xw[0], self.ycp, layers=[0]
            ) + self.model.head(0, 0, layers=[0])
            self.diff += self.hls - self.hmax
            msg = r"hmax={0:.2f}, hls={1:.2f}: Offset hls by {2:.3f}".format(
                self.hmax[0], self.hls[0], self.diff[0]
            )
            self.logger.debug(msg)
        print()
        print("solution complete")
        self.logger.info(
            "Time elapsed: {0:.1f} seconds".format(
                (pd.Timestamp.now() - self.xsm.init_time).total_seconds()
            )
        )
    ...
    def build_3D_timml_model(self, ls):
        ...
        inhom_riv_vp = timml.StripInhomMaq(
            mlvp,
            x1=-np.inf,
            x2=self.xsm.xR_riv,
            kaq=self.xsm.kh,
            z=self.xsm.z,
            c=np.r_[np.atleast_1d(self.xsm.criv), self.xsm.caq],
            npor=0.3,
            topboundary="semi",
            hstar=0.0,
        )
        inhom_voorland_vp = timml.StripInhomMaq(
            mlvp,
            x1=self.xsm.xR_riv,
            x2=self.xsm.xR_voorland,
            kaq=self.xsm.kh,
            z=self.xsm.z,
            npor=0.3,
        ...
        inhom_achterland_vp = timml.StripInhomMaq(
            mlvp,
            x1=self.xsm.xR_voorland,
            x2=np.inf,
            kaq=self.xsm.kh,
            z=self.xsm.z,
            topboundary="semi",
        ...
        ylim = self.L_total / 2.0 + 3 * maxlab
        dymin = np.amin([200.0, maxlab])
        rest = ylim - 3 * maxlab
        ypts = np.cumsum(
            np.r_[
                dymin * np.ones(int((np.ceil(rest / dymin)))),
                np.array([maxlab / 2.0, maxlab / 2.0, maxlab, maxlab]),
            ]
        )
        y = np.r_[-1 * ypts[::-1], np.zeros(1), ypts]

        self.model1d_with_2d_elements(
            mlvp,
            [inhom_riv_vp, inhom_voorland_vp, inhom_achterland_vp],
            y,
            order=3,
            ndeg=3,
        )
        ...
        for i in self.wlayers:
            timml.LineSinkBase(
                ...
                x2=self.xw[0],
                y2=self.L_total / 2.0,
                Qls=-1 * ls.parameters[i] * self.L_total,
                layers=[i],
            )
    ...
    def plot_vp(self, ax):
        ax.plot(
            [0, 0],
            [self.xsm.z_deklaag_onder - self.L_filter, self.xsm.z_deklaag_onder],
            c="k",
            lw=4,
            label="verticale put",
            ls="dotted",
    ...
    def seepage(self):
        def fqvp(y):
            return np.sum(
                self.model.disvec(self.xsm.xR_voorland, y + 1e-10)
                + self.model2.disvec(self.xsm.xR_voorland, y + 1e-10)
            )

        fn = self.L_total / self.welldist
        Q = quad(fqvp, -self.welldist / 2.0, self.welldist / 2.0)[0] * fn
        return Q
    ...
    def headinwells(self):
        ...
        return np.array(
            [
                i.headinside()[0] + self.model.head(self.xw[0], 0, 0)
    ...
    def velocity_at_well(self, rw=None, porosity=None):
        ...
        if rw is None:
            rw = self.rw
        return self.discharge() / (2 * np.pi * rw / 2.0 * self.L_filter * porosity)
    ...
    def check_pump_needed(self):
        b = (
            self.headalongline([self.xw[0] - self.rw], [self.yw[0]], layers=0)[0, 0]
            > self.xsm.hpolder
        )
        if b:
            msg = "Vrije afwatering WEL mogelijk! H_drainage {0:.2f} > H_polder {1:.2f}"
        ...
        self.logger.info(
            msg.format(
                self.headalongline([self.xw[0] - self.rw], [self.yw[0]], layers=0)[
                    0, 0
                ],
                self.xsm.hpolder,
            )
