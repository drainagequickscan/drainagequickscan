...
xg = np.r_[np.linspace(xsm_vp.xsm.xR_voorland - 200., xsm_vp.xsm.xR_voorland - 50., 21),
           ...
           np.linspace(xsm_vp.xsm.xR_voorland + 75., xsm_vp.xsm.xR_voorland + 100., 21)[1:]]
yg = np.linspace(-45, 45, 101)
hgr = xsm_vp.headgrid(xg, yg, layers=[0])

# %% Top view with contours final result
fig, ax = xsm_vp.plots.topview(xlim=[-180, 100], ylim=[-100, 100], inhom_bounds=True, add_wlvl=True)
...
xg1 = np.linspace(xsm_vp.xsm.xR_voorland - 200., 100., 101)
...
_, ax0 = xsm_vp.plots.topview_nodike(xlim=[-180, 100], ylim=[-45, 45], ax=ax0, inhom_bounds=True, labels=False)
_, ax1 = xsm_vp.plots.topview_nodike(xlim=[-180, 100], ylim=[-45, 45], ax=ax1, inhom_bounds=True, labels=False)
_, ax2 = xsm_vp.plots.topview_nodike(xlim=[-180, 100], ylim=[-45, 45], ax=ax2, inhom_bounds=True, add_wlvl=True)

# Plot 3
p2 = ax1.axvline(0., linestyle="dashed", color="green", linewidth=4, label="lijn injectie")
p1, = ax2.plot(xsm_vp.xw, xsm_vp.yw, c="k", marker="o", ls="none", label="verticale bronnen", markersize=8)
levels = np.r_[np.arange(1.8, 3.0, 0.1), np.arange(3.0, 6.5, 0.5)]
cs = ax2.contour(xg, yg, hgr[0], levels, cmap="RdYlBu", vmin=2.5, vmax=6.2, linewidths=2)
plt.clabel(cs, fmt="%.1f m", inline=True, inline_spacing=-10)
# ax2.plot([], [], ls="dashed", c="gray", lw=1, label="grenzen tussen \nzones in model")


# Plot 1
p0 = ax0.axvline(0., linestyle="dashed", color="green", linewidth=4, label="oneindige lange \nonttrekking")
# ax0.plot([], [], ls="dashed", c="gray", lw=1, label="grenzen tussen \nzones in model")

# contours
levels1 = np.arange(2.8, wdict["hriv"] - 0.3, 0.3)
cs0 = ax0.contour(xg1, yg1, hgr1[0], levels1, cmap="RdYlBu", linewidths=2, vmin=2.5, vmax=6.2)
...
ax0.text((xsm_vp.xsm.xR_voorland + 100) / 2., 0, "achterland \n(NAP{0:+.1f}m)".format(xsm_vp.xsm.hpolder),
         rotation=0, ha="center", va="center")

# Plot 2
ax1.plot(xsm_vp.xw, xsm_vp.yw, c="k", marker=".", ls="none", label="verticale bronnen", zorder=11)
# ax1.plot([], [], ls="dashed", c="gray", lw=1, label="grenzen tussen \nzones in model")

# custom labels for zones
ax1.text((-180 + xsm_vp.xsm.xL_voorland) / 2., 0, "oppervlaktewater \n(NAP{0:+.1f}m)".format(0.0),
         rotation=270, ha="center", va="center")
ax1.text((xsm_vp.xsm.xR_voorland + xsm_vp.xsm.xL_voorland) / 2., 0, "voorland \n(NAP{0:+.1f}m)".format(0.0),
         rotation=0, ha="center", va="center")
ax1.text((xsm_vp.xsm.xR_voorland + 100) / 2., 0, "achterland \n(NAP{0:+.1f}m)".format(0.0),
         rotation=0, ha="center", va="center")

# contours
levels2 = np.r_[np.arange(-1.0, 0.0, 0.05), np.arange(0.05, 0.2, 0.05)]
cs1 = ax1.contour(xg2, yg2, hgr2[0], levels2, cmap="seismic_r", vmin=-0.5, vmax=0.5, zorder=10, linewidths=2)
...
for iax in [ax0, ax1, ax2]:
    ...
    iax.axvspan(-xsm_vp.xsm.breedte_dijk_totaal, 0.0, hatch="\/", color="k", label="dijk", fill=False,
                linestyle="dashed", zorder=1, alpha=0.45)

p = mpl.patches.Patch(color="k", alpha=0.25, hatch=r"\\\///", linestyle="dashed", fill=False)
...
ax1.set_xlabel("")
ax1.legend([p1, p2, p], ["verticale bronnen", "lijn injectie", "zone dijk"], loc="upper right")
